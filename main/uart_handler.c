...
UART_Setup ()
{
    bq79600_wake_tone (); // Wake up the transceiver module
...
/**
 * Calculates CRC-16-IBM for BQ79600 communication
...
esp_err_t
bq_uart_tx_rx (const uint8_t *tx_data, size_t tx_len, uint8_t *rx_data, int rx_len)
{

    uart_write_bytes (UART_PORT_NUM, (const char *)tx_data, tx_len);
    uart_wait_tx_done (UART_PORT_NUM, pdMS_TO_TICKS (20));

    if (rx_len > 0)
	{
	    int len = uart_read_bytes (UART_PORT_NUM, rx_data, rx_len, pdMS_TO_TICKS (200));
	    if (len != rx_len)
		{
		    ESP_LOGE (TAG, "Expected %d Bytes, got %d Bytes", rx_len, len);
		    return ESP_FAIL;
		}
