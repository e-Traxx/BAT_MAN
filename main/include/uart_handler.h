...
typedef struct __attribute__ ((packed))
{
    ...
    uint8_t reg_msb;
    uint8_t reg_lsb;
    uint8_t data_len;	       // number of meaningful bytes in data[]
    uint8_t data[MAX_TI_DATA]; // actual payload or response
    uint8_t crc[2];	       // CRC-16-IBM, big-endian
} TI_Frame_t;

typedef union
{
    TI_Frame_t frame;
    uint8_t raw[sizeof (TI_Frame_t)];
} Robin_TI_Union_t;

extern Robin_TI_Union_t Robin_frame;
