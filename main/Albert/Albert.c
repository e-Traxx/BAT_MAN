 ...
 * - The HTFS-200p sensor is used to measure the current flowing through the TSAC main HV wires.
 * - A user-defined table maps sensor voltage values to corresponding current values.
 * - Linear interpolation is applied to calculate the current based on the sensor's output.
 * - The measured current is logged and stored for further processing.
 */

void Albert_Setup() {
    /* Check if the Sensor is Connected / Check for Open Wire
    * The sensor always lets about 1.25V through, so if the sensor is disconnected, the voltage will be 0V or lower than 2.5V.
    */
    ...
    esp_adc_cal_value_t V_out = esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTEN_DB_11, ADC_WIDTH_BIT_12, 1100, &adc_chars);

    // Check if the Sensor is Connected / Check for Open Wire
    int adc_reading = adc1_get_raw(ADC_CHANNEL);
    uint32_t voltage = esp_adc_cal_raw_to_voltage(adc_reading, &adc_chars);

    // If the voltage is lower than 1.25V, the sensor is not connected or there is an open wire
    if (voltage < 1000) {
...
void Albert_timer_callback(TimerHandle_t AlbertTimer) {
    ...
    if (xSemaphoreGiveFromISR(albert_semaphore, &HighPriorityTaskWoken) != pdPASS) {
